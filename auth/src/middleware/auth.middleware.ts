...
const authMw = async (req: CustomExpressRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const re = responseWrapper(false, COMMON_MESSAGE.Unauthorized, 401, null, null);
    res.status(401);
    if (!token) return res.json(re);
    const cb = (err: JsonWebTokenError, decoded: CustomJwtPayload) => {
      if (err && err.name === 'TokenExpiredError') return refreshToken(req, res, next);
      else if (err) return res.json(responseWrapper(false, COMMON_MESSAGE.Unauthorized, 401, null, err));
      res.status(200);
      req.token = decoded;
      return next();
    };
...
const refreshToken = (req: CustomExpressRequest, res: Response, next: NextFunction) => {
  const refreshToken = req.header('Refresh-Token')?.replace('Bearer ', '');
  const cb = (err: JsonWebTokenError, payload: CustomJwtPayload) => {
    res.status(401);
    if (err) return res.json(responseWrapper(false, COMMON_MESSAGE.Unauthorized, 401, undefined, err));
    res.status(200);
